"""
Flask Stream 14 - Intelligent Advertisement Filtering
Key improvements over v13:
- Built-in ad filtering for magazine PDFs
- Pre-configured German motorcycle magazine ad keywords
- Configurable filter controls (word count, page position)
- Real-time filter statistics
- All v13 features included (Edge browser fix, etc.)
"""
...
# German motorcycle magazine ad keywords (optimized for Tourenfahrer, Motorrad, Bike, etc.)
AD_KEYWORDS = [
    # German ad indicators
    'anzeige', 'werbung', 'inserat', 'promotion', 'sponsored',
    'advertorial', 'werbebeitrag', 'partnerbeitrag',

    # Pricing indicators (ads often have prices)
    'preis ab', 'ab €', 'uvp', 'statt €', 'rabatt',
    'sonderpreis', 'aktion', '% reduz', 'prozent',
    'sparen', 'günstig', 'angebot',

    # Sales language
    'jetzt kaufen', 'bestellen', 'hier bestellen',
    'in den warenkorb', 'zum shop', 'jetzt bestellen',

    # Contact/dealer info (common in ads)
    'händler', 'händlerverzeichnis', 'vertrieb',
    'info@', 'kontakt@', 'tel:', 'fax:', 'telefon:',

    # Classified ads
    'zu verkaufen', 'verkaufe', 'biete', 'suche',
    'kleinanzeigen', 'marktplatz', 'gebraucht',

    # Product catalog language
    'lieferbar', 'verfügbar', 'bestellnummer',
    'artikel-nr', 'art.-nr', 'bestell-nr',

    # Common advertising phrases
    'mehr infos', 'weitere informationen', 'hier klicken',
    'besuchen sie', 'fordern sie an',
]

# Default filter settings
DEFAULT_MIN_WORDS = 150  # Articles are typically longer than ads
DEFAULT_SKIP_FIRST_PAGES = 2  # Often contain cover ads, table of contents ads
DEFAULT_SKIP_LAST_PAGES = 2  # Often contain classifieds, back cover ads
...
def is_likely_advertisement(text, min_words=150, ad_keywords=AD_KEYWORDS):
    """
    Detect if text is likely an advertisement using multiple heuristics

    Args:
        text: Text to analyze
        min_words: Minimum word count for articles (vs ads)
    ...
    # Check 5: Suspicious call-to-action patterns
    suspicious_patterns = [
...
def filter_advertisement_results(results, enable_filter=True, min_words=150,
    ...
    for result in results:
        ...
        # Filter by page position (first/last pages often have ads)
        if skip_first_pages > 0 and page_number <= skip_first_pages:
            stats['filtered_position'] += 1
            continue

        # Note: Can't easily filter last pages without total page count per PDF
        # This would require tracking total pages in the cache
    ...
    if show_stats and (stats['filtered_ads'] > 0 or stats['filtered_position'] > 0):
        ...
        st.info(
            f"🚫 **Ad Filter:** Removed {total_filtered} of {stats['total']} results ({percentage:.1f}%) "
            f"• {stats['filtered_ads']} ads • {stats['filtered_position']} by position"
        )

        if stats['reasons']:
            with st.expander("📊 See filtering details"):
                st.write("**Why pages were filtered:**")
                for reason, count in sorted(stats['reasons'].items(), key=lambda x: -x[1]):
                    st.write(f"- {reason}: {count} pages")
...
def main():
    ...
    st.set_page_config(
        page_title="PDF Keyword Search v14 (Ad Filtering)",
        layout="wide",
    ...
    st.title("🔍 PDF Keyword Search v14 (Ad Filtering)")
    st.write("Search through your PDF magazines with intelligent ad filtering - focus on actual content!")
    ...
    if keyword and (search_button or keyword):
        ...
        enable_ad_filter = st.session_state.get('enable_ad_filter', True)
        min_words = st.session_state.get('min_words', DEFAULT_MIN_WORDS)
        skip_first = st.session_state.get('skip_first', DEFAULT_SKIP_FIRST_PAGES)
        skip_last = st.session_state.get('skip_last', DEFAULT_SKIP_LAST_PAGES)
    ...
    with st.sidebar:
        ...
        if enable_ad_filter:
            ...
            with col1:
                skip_first = st.number_input(
                    ...
                    min_value=0,
                    max_value=10,
                    value=DEFAULT_SKIP_FIRST_PAGES,
            ...
            with col2:
                skip_last = st.number_input(
                    ...
                    min_value=0,
                    max_value=10,
                    value=DEFAULT_SKIP_LAST_PAGES,
                    help="Often contain classifieds",
                    key="skip_last"
                )

            # Info about the filter
            with st.expander("ℹ️ How ad filtering works"):
                st.markdown("""
                **Ad Detection Methods:**
                - Word count (ads are shorter)
                - German ad keywords (anzeige, werbung, etc.)
                - Price density (€ symbols)
                - Contact info (tel:, email, www.)
                - Call-to-action phrases

                **Optimized for:**
                - Tourenfahrer, Motorrad, Bike
                - German motorcycle magazines
                - Travel reports vs tour ads
                - Test rides vs product ads
                """)
        ...
        **Version 14 Features:**
        ...
        - ✅ Pre-configured for German mags
